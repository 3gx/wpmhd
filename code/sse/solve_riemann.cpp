...
ptcl_mhd system::solve_riemann(const float3 &Wij, const float3 &Bij, 
  ...
  const float  ds   = std::sqrt(e.x*e.x + e.y*e.y);
  ...
  F.dens  = Fdens;
  F.ethm  = Fetot;
  F.vel.x = iAxx*Fmomx + iAxy*Fmomy + iAxz*Fmomz;
  F.vel.y = iAyx*Fmomx + iAyy*Fmomy + iAyz*Fmomz;
  F.vel.z = iAzx*Fmomx + iAzy*Fmomy + iAzz*Fmomz;
  F.B.x   = iAxx*Fbx   + iAxy*Fby   + iAxz*Fbz;
  F.B.y   = iAyx*Fbx   + iAyy*Fby   + iAyz*Fbz;
  F.B.z   = iAzx*Fbx   + iAzy*Fby +   iAzz*Fbz;
  F.psi   = Fpsi;
...
// #endif
  ...
  const double cfl = std::sqrt((gpbl + std::sqrt( sqr(gpbl) - 4*gpl*sqr(Bx) ))/(2.0*dens_L));
  const double cfr = std::sqrt((gpbr + std::sqrt( sqr(gpbr) - 4*gpr*sqr(Bx) ))/(2.0*dens_R));
  const double cfmax = std::sqrt(1.0)*std::max(cfl,cfr);
...
#error "Please make sure this code is not compiled in solve_riemann.cpp"
  ...
  double dsl = std::sqrt(dens_L);
  double dsr = std::sqrt(dens_R);
  ...
  double cf2_cs2 = std::sqrt(sqr(tdif) + 4.0*twid_asq*ct2);
  ...
  double asq = std::sqrt(gamma_gas * pres_L/dens_L);
  vaxsq = sqr(Bx)/dens_L;
  ct2   = (sqr(By_L) + sqr(Bz_L))/dens_L;
  double qsq = vaxsq + ct2 + asq;
  double tmp = vaxsq + ct2 - asq;
  cfsq = 0.5*(qsq + std::sqrt(sqr(tmp) + 4.0*asq*ct2));
  double cfl = fsig*std::sqrt(cfsq);

  asq = std::sqrt(gamma_gas * pres_R/dens_R);
  vaxsq = sqr(Bx)/dens_R;
  ct2   = (sqr(By_R) + sqr(Bz_R))/dens_R;
  qsq = vaxsq + ct2 + asq;
  tmp = vaxsq + ct2 - asq;
  cfsq = 0.5*(qsq + std::sqrt(sqr(tmp) + 4.0*asq*ct2));
  double cfr = fsig*std::sqrt(cfsq);
  ...
  const double    S_L_s  = S_M - std::abs(Bx)/std::sqrt(dens_L_s);
  const double    S_R_s  = S_M + std::abs(Bx)/std::sqrt(dens_R_s);

  const double idsqrt  = 1.0/(std::sqrt(dens_L_s) + std::sqrt(dens_R_s));
  const double  vely_ss = idsqrt*(std::sqrt(dens_L_s)*vely_L_s + std::sqrt(dens_R_s)*vely_R_s + (By_R_s - By_L_s)*signBx);
  const double  velz_ss = idsqrt*(std::sqrt(dens_L_s)*velz_L_s + std::sqrt(dens_R_s)*velz_R_s + (Bz_R_s - Bz_L_s)*signBx);

  const double By_ss = idsqrt*(std::sqrt(dens_L_s)*By_R_s + std::sqrt(dens_R_s)*By_L_s + std::sqrt(dens_L_s*dens_R_s)*(vely_R_s - vely_L_s)*signBx);
  const double Bz_ss = idsqrt*(std::sqrt(dens_L_s)*Bz_R_s + std::sqrt(dens_R_s)*Bz_L_s + std::sqrt(dens_L_s*dens_R_s)*(velz_R_s - velz_L_s)*signBx);
  ...
  const double etot_L_ss = etot_L_s - std::sqrt(dens_L_s)*(vB_L_s - vB_L_ss)*signBx;

  const double vB_R_ss   = velx_R_ss*Bx + vely_R_ss*By_R_ss + velz_R_ss*Bz_R_ss;
  const double etot_R_ss = etot_R_s + std::sqrt(dens_R_s)*(vB_R_s - vB_R_ss)*signBx;
...
#if 0
  const real dpsi = psi_L - psi_R;
  const real Sx = (Bx_L  == Bx_R ) ? HUGE*sign(Bx_L *velx_L - Bx_R *velx_R + dpsi) : (Bx_L *velx_L - Bx_R *velx_R)/(Bx_L  - Bx_R);
  ...
  Fbx  = (Sx >= w) ? Bx_L*(velx_L - w) + psi_L : Bx_R*(velx_R - w) + psi_R;
