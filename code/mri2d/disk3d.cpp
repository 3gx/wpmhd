...
#else
  const int   nmode  = 4;
  const float b0     = 0.005513f/nmode;
  float bx =  0.0f;
  float by =  0.0f;
  float bz =  0.0f;
...
#if 1
  const float Rin  = 1.25;
  const float Rout = 3.75;
  if (R < Rin || R > Rout) {
...
void system::setup_particles(const bool init_data) {
  ...
  const float3 rmax = (float3){20.0f, 20.0f,  8.0f};
...
//     const int np = 100000;
//     const float Lx = rmax.x - rmin.x;
//     const float Ly = rmax.y - rmin.y;
//     const float Lz = rmax.z - rmin.z;
//     const float h = powf(Lx*Ly*Lz/np, 1.0/3);
//     for (int i = 0; i < np; i++) {
//       particle p;
//       const float xp = drand48() * Lx;
//       const float yp = drand48() * Ly;
//       const float zp = drand48() * Lz;
...
// 	}
	...
	const float height = (2.0*drand48() - 1.0) * 2*H;
