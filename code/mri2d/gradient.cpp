...
void system::gradient() {
  ...
  for (int leaf = 0; leaf < n_leafs; leaf++) {
    ...
    for (octbody *ibp = inode.pfirst; ibp != NULL; ibp = ibp->next) {
      ...
      std::vector<int     > shear_dr;
      jlist_pmhd.reserve(256);
      jlist_dr.reserve(256);
      shear_dr.reserve(256);
 	...
 	for (octbody *jbp = jnode.pfirst; jbp != NULL; jbp = jbp->next) {
      ...
      ngb_min = std::min(ngb_min, nj);
      ngb_max = std::max(ngb_max, nj);
      ngb_sum += nj;
      
      for (int j = 0; j < nj; j++) {
	ptcl_mhd mj = jlist_pmhd[j];
	...
	mj.vel.z -= shear_dr[j]*qOmega*Omega*global_domain.hsize.x.getu();
