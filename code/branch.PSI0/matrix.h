...
public:
  ...
  void comparetoidentity()  {
    ...
    std::cerr << "Worst diagonal value deviation from unity: " 
	      << maxunitydeviation << " at row/column " << worstdiagonal << std::endl;
    std::cerr << "Worst off-diagonal value deviation from zero: " 
  ...
  D& operator() (int column, int row) {
    return data[ row * maxsize + column];
  }

  bool invert()  {
    if (actualsize <= 0) return false;  // sanity check
    if (actualsize == 1) return false;  // must be of dimension >= 2
	...
	if (data[i*maxsize + i] == 0.0) return false;
        data[i*maxsize+j] = 
	...
	if (data[j*maxsize + j] == 0.0) return false;
