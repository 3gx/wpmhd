...
#define _MACH_ 100.0f
// #define _BETA_ 1000.0f;
...
#else
  const float Omega  = sqrt(gravity_mass/(R*R*R));
  const float lambda = global_domain.hsize.z.getu()/1.0;
  const float beta   = _BETA_;
  const float ff     = 2*M_PI*sqrt(32*gamma_gas/15.0/beta);
  const float cs     = Omega*lambda/ff;
#endif
