...
float4 system::body_forces(const pfloat3 pos) {
  ...
  return float4(-mids3*dr.x, -mids3*dr.y, -0, -mids);
};
...
void system::boundary_particles(const int idx) {
  ...
  if (iteration == 50 && !do_magnetic) {
    ...
    const float beta = 1348;
    const float pres = compute_pressure(mi.dens, mi.ethm);
    const float b0   = sqrt(2*pres/beta);
    
    float bx = 0;
    float by = 0;
    float bz = 0;

    const float B_Rin  = 40 + 2.0f;
    const float B_Rout = 40 + 3.0f;
...
void system::setup_particles(const bool init_data) {
  ...
  const float3 rmax = (float3){100.0f, 100.0f,  1.0f};
...
//   const float cs0 = _CS0_;
  ...
  const float Rin  = 40 + 1.0f;
  const float Rout = 40 + 4.0f;
  ...
  gravity_mass = std::pow(40.0, 3);
  gravity_eps2 = sqr(0.0f);
  gravity_rin  = (Rin  - 40)*0.9 + 40;
  gravity_rout = (Rout - 40)*1.1 + 40;
...
//   nt = (int3){2,2,2};
  ...
  if (myid == 0) {
	...
	const float h  = std::pow(M_PI*(sqr(Rjp) - sqr(Rj))*2*1/Nj, 1.0/3);
	p.h = 3.0*h;
