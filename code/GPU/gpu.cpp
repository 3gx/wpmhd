...
void gpu_struct::load_kernels() {
...
#if 1
  const char *flags = 
    "-DNMAXPERLANE=16 -DNDIM=2 -D_PERIODIC_BOUNDARIES_=1 -DNGBMAX=192 -DNBLOCKDIM=64 -I./CLkernels";
#else
  const char *flags = 
    "-DNMAXPERLANE=16 -DNDIM=3 -DNGBMAX=160 -DNBLOCKDIM=64 -I./CLkernels";
#endif

#else
  const char *flags = 
    "-DNMAXPERLANE=16 -DNDIM=2 -D_PERIODIC_BOUNDARIES_=3 -DNGBMAX=256 -DNBLOCKDIM=64 -I/Users/egaburov/work/papers/GNcode/GPU/CLkernels";
  ...
  compute_wngb.load_source("CLkernels/compute_wngb.lanes.cl", "", flags, maxrregcount);
  compute_wngb.create("compute_wngb");

  fprintf(stderr, "Loading compute_wngb_dr kernel ...\n");
  compute_wngb_dr.setContext(context);
  compute_wngb_dr.load_source("CLkernels/compute_wngb_dr.lanes.cl", "", flags, maxrregcount);
  compute_wngb_dr.create("compute_wngb_dr");
...
void gpu_struct::init() {
  ...
  dwdt.setContext(context);

  nngb.setContext(context);
  nj_gather.setContext(context);
  nj_both.setContext(context);
  ...
  in_group_bodies.setContext(context);
  in_bodyIds.setContext(context);

  in_ilist.setContext(context);
  in_jlist.setContext(context);
  in_joffset.setContext(context);
  jidx.setContext(context);
  nj.setContext(context);
