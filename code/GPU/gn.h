...
#define  TREE_NLEAF 128
#define  DOMAIN_TREE_NLEAF 128
...
public:
  ...
  double t_nngb, t_renorm, t_gradient, t_mhd_interaction;
  ...
  double t_weight,  t_grad, t_interaction, t_tree;
  double t_import_bnd;
  ...
  bool do_kepler_drift;
  bool eulerian_mode, do_morton_order, do_ppm, do_adjust_particles;
...
public:
  ...
  system(int nproc, int myid, bool ppm = false, int ndim = 3)  {
    ...
    constB = (float3){0.0f, 0.0f, 0.0f};
  };
  ~system() {};


  void set_parallel_domain();
  void set_geometry(const bool init_data);
  void set_problem(const bool init_data);
  void boundary_particles(const int idx);
  void boundary_derivatives(const int idx);
  ...
  void build_tree();
  void ngb_search();
  void adjust_positions(const int, const float);
  void compute_weights();
  void compute_weights    (std::vector<octnode<TREE_NLEAF>*> &, const float);  
  void compute_weights_cl();
  void compute_weights_cl (std::vector<octnode<TREE_NLEAF>*> &, const float);  
  ...
  void push_particles();
  void derivs(const bool flag);
  bool do_first_order, do_predictor_step;
  void iterate();
  void vain();
  void compute_dt();
