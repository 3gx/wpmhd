...
//  	    const float wki = 1;//wi; //*dr.x; //wi; //inv_hidim * wi;
//  	    const float wki = -1;//dr.x; //*fabs(dr.x);
// 	    const float wki = -dr.x; //(s2 > 0.0f) ? -1.0f : 0.0f;
...
// 	    const float3 eij = {dwij[0] * iwij, dwij[1] * iwij, dwij[2] * iwij};
	    const float3 eij = {dwij[0], dwij[1], dwij[2]};
	    
	    float Bm, psiM;
	    ptcl_mhd Fij = solve_riemann(Wij, Bij, Vij, Qi, Qj, eij, Bm, psiM);
// 	    Fij.mom.x = dwi[0]; //wki*Bxx[li]*wi*(dr.x); //dwi[0]; //-dwij[0];
//  	    if (pi.local_idx == pj.local_idx) 
//  	      Fij.mom.x = 0;
      ...
      const float4 acc = body_forces(ipos);
      dot.mom.x += acc.x*m;
      dot.mom.y += acc.y*m;
      dot.mom.z += acc.z*m;
