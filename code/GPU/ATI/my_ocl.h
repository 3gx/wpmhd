...
//   protected:
  ...
  public:
	...
	ciErrNum = clGetPlatformIDs(numPlatforms, &platforms[0], NULL);
	oclCheckError(ciErrNum);
...
/*      unsigned int free, total;
  ...
  public:
    ...
    void cmalloc(int n, bool pinned = false,  cl_mem_flags flags = CL_MEM_READ_WRITE) {
      ...
      cl_int err;
      if (flags & (CL_MEM_USE_HOST_PTR == 0)) hDeviceMem = clCreateBuffer(hContext, flags, size*sizeof(T), &host_ptr[0], &err);
      else                             	      hDeviceMem = clCreateBuffer(hContext, flags, size*sizeof(T), NULL, &err);
      oclCheckError(err);
