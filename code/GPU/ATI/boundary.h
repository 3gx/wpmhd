...
struct boundary {
  ...
  boundary(const float3 pos, const float h1, const float scale_factor = 1.0f) {
    float h = h1*scale_factor*SMALLB;
    centre.x.set(pos.x);
    centre.y.set(pos.y);
    centre.z.set(pos.z);
    hsize.x.set(h);
  ...
  boundary(const float4 pos, const float scale_factor = 1.0f) {
...
#if 1
    const bool flag1 = dr.x.aleq(ds.x) &&  dr.y.aleq(ds.y) &&  dr.z.aleq(ds.z);
    const bool flag2 = b.hsize.x.aleq(hsize.x) && b.hsize.y.aleq(hsize.y) && b.hsize.z.aleq(hsize.z);
    return flag1 && flag2;
#else
    return (dr.x.aleq(ds.x) &&  dr.y.aleq(ds.y) &&  dr.z.aleq(ds.z));
