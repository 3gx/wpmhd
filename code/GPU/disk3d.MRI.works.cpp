...
float system::compute_pressure(const float dens, const float ethm) {
  ...
  return sqr(cs0) * dens; ///(gamma_gas - 1.0f);
}
...
bool system::remove_particles_within_racc(const int idx) {
  ...
  bool flag = ds2 <= sqr(gravity_rin) || ds2 >= sqr(gravity_rout);
  if (flag) {
    local_n--;
...
void system::boundary_particles(const int idx) {
  pmhd[idx] = pmhd[idx].to_primitive(pvec[idx].wght);

  pmhd[idx].ethm = compute_pressure(pmhd[idx].dens, 1.0); ///(gamma_gas - 1.0);
  
  static bool do_magnetic = false;
  if (iteration == 100 && !do_magnetic) {
...
// #define _TOROIDAL_
    ...
    const float pres = compute_pressure(pmhd[idx].dens, pmhd[idx].ethm);
  ...
  float z = pos.z.getu() - gravity_pos.z;
  const float Rin  = 1.0;
  const float Rout = 4.0;
  float r = sqrt(x*x + y*y + z*z);
  if (r < Rin || r > Rout) {
    pmhd[idx].B = (real3){0.0f, 0.0f, 0.0f};
    pmhd[idx].psi = 0.0f;
  }
  

  pmhd[idx] = pmhd[idx].to_conservative(pvec[idx].wght);
  return;
...
//   nt = (int3){2,2,2};
  ...
  if (myid == 0) {
	...
	flag  = true;
//  	fprintf(stderr, "ring= %d Rj= %g  Rjp= %g  r= %g H= %g\n", j, Rj, Rjp ,r, H);
	float height;
	while (flag) {
...
// 	height *= 0.5;
// 	height = (2.0*drand48() - 1)*r;
	particle p;

	float h  = std::pow(M_PI*(sqr(Rjp) - sqr(Rj))*2*H/Nj, 1.0/3);
	p.h = 3.0*h;
    ...
    for (int pc = 0; pc < n; pc++) {
      ...
      const float vcirc = sqrt(Mbh/r);
      const float H = cs0/vcirc * r;

      p.dens = exp(-sqr(z)/2/sqr(H)); //powf(r, -sig_slope);
      p.ethm = sqr(cs0) * p.dens /(gamma_gas - 1.0)/gamma_gas;
