...
//   s.eulerian_mode   = true;
  ...
  s.set_parallel_domain();
  
  if (argc > 2) {

    if (myid == 0) fprintf(stderr, "  reading snapshot ... \n");
    load_data = true;
    ...
    if (myid == 0) fprintf(stderr, "  setting up data ... \n");
    s.set_geometry(true);
    char fnn[256];
    sprintf(fnn, "%s/geom_p%.3d.bin", path, s.myid);

    s.dump_binary(fnn, false);
    s.set_problem(true);

    char fnn1[256];
    sprintf(fnn1, "%s/init_p%.3d.bin", path, s.myid);
    fprintf(stderr, " dump init to %s \n",  fnn1);
    s.dump_binary(fnn1, true);
...
//       if (s.iteration%2 == 0) {
	...
	fprintf(stderr, "proc= %d: iter= %d: t= %g dt = %g cons= (%g %g %g) diff= (%g %g %g) Ethm= %g Ekin= %g Emag= %g  done in %g sec.\n",
		...
		Mtot, Etot, Volume, 
		(Mtot - Mtot0)/Mtot0, (Etot - Etot0)/Etot0, (Volume - Volume0)/Volume0, Ethm, Ekin - 0*Ekin0, Emag, dt_done);
//       } else {
