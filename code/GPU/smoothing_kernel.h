...
struct smoothing_kernel {
  ...
  void set_dim(int NDIM) {
    ...
    case 2:
      coeff          =  1.0f/0.549779;
      NORM_COEFF     =  M_PI * coeff;
      break;
    default:
      coeff          =  1.0f/0.392699;
      NORM_COEFF     =  4.0*M_PI/3;
    }
  }
  
  float w(float u) {
    const float q  = 2.0f*u;
    const float q1 = 1.0f - (1.5f*q - 0.75f*q*q)*q;
    const float q2 = 2.0f - q;
    const float q3 = 0.25f*q2*q2*q2;
    const float wk = ((q < 1.0f) ? q1 : ((q < 2.0f) ? q3 : 0.0f));
    return wk;
  }
  float w4(float u) {return w(u);}
  ...
  float dw(float u) {
    const float q  = 2.0f*u;
    const float q1 = (-3.0f + 2.25f*q)*q*2.0f;
    const float q2 = 2.0f - q;
    const float q3 = -0.75f*q2*q2*2.0f;
    const float wk  = ((q < 1.0f) ? q1 : ((q < 2.0f) ? q3 : 0.0f))*coeff;
    return wk;
  }
