...
//   protected:
  ...
  public:
    ...
    int create(bool disableTiming = false, cl_device_type device_type = CL_DEVICE_TYPE_GPU) {
      ...
      oclSafeCall(clGetDeviceIDs (clPlatform, this->device_type, 0, NULL, &ciDeviceCount));
     
      hDevices = (cl_device_id*) malloc(sizeof(cl_device_id)*ciDeviceCount);
      oclSafeCall(clGetDeviceIDs (clPlatform, this->device_type, ciDeviceCount, hDevices, &ciDeviceCount));
    ...
    void createQueue(size_t dev = 0) {
      ...
      assert(!hContext_flag);
      hContext = clCreateContext(0, 1, &hDevices[dev], NULL, NULL, &ciErrNum);
      this->dev = dev;
...
/*      CU_SAFE_CALL(cuEventRecord(stop, stream));
      ...
      if(logfile_flag)
      {
        (*logFile) << logID++ << "\t"  << type << "\t" << text << "\t" << time << std::endl;
      }
    }    
...
/*      unsigned int free, total;
  ...
  public:
    ...
    void cmalloc(int n, bool pinned = false,  cl_mem_flags flags = CL_MEM_READ_WRITE) {
      ...
      host_ptr.resize(len);
      cl_int err;
      if (flags & (CL_MEM_USE_HOST_PTR == 0)) hDeviceMem = clCreateBuffer(hContext, flags, len*sizeof(T), &host_ptr[0], &err);
      else                             	      hDeviceMem = clCreateBuffer(hContext, flags, len*sizeof(T), NULL, &err);
      oclCheckError(err);
    ...
    void ccalloc(int n, bool pinned = false,  cl_mem_flags flags = CL_MEM_READ_WRITE) {
      ...
      len = n;
      host_ptr.resize(len);      
      memset(&host_ptr[0], 0, len*sizeof(T));   
	...
	hDeviceMem = clCreateBuffer(hContext, flags, len*sizeof(T), &host_ptr[0], &err);
      }
      oclCheckError(err);
      hDeviceMem_flag = true;
    }
    ...
    void zeroMem()
    {
      assert(context_flag);
      assert(hDeviceMem_flag);
      memset(&host_ptr[0], 0, len*sizeof(T));    //Set the vector to value 0 
...
//       fprintf(stderr, "finalBuildOptions= %s\n", finalBuildOptions);
    ...
    void print_compiler_output() {
      ...
      char buildLog[10240];
      clGetProgramBuildInfo(hProgram, hDeviceId, CL_PROGRAM_BUILD_LOG, sizeof(buildLog), buildLog, NULL);
    ...
    void setWork(const int items, const int n_threads, const int blocks = -1) {
      if      (blocks == -1  ) setWork_2D(items, n_threads);
      else if (blocks > 65536) setWork_block2D(n_threads, blocks);
      else                     setWork_block1D(n_threads, blocks);
    }    
    ...
    void setWork_1D(const int items, const int n_threads){
      std::vector<size_t> localWork(2), globalWork(2);
      const int ng = (items) / n_threads + 1;
      const int nx = ng;
      const int ny = 1;
      globalWork[0] = nx*n_threads;  globalWork[1] = ny;
      localWork [0] = n_threads;      localWork[1] = 1;   
    ...
    void setWork_2D(const int items, const int n_threads) {
      std::vector<size_t> localWork(2), globalWork(2);
      const int ng = (items) / n_threads + 1;
    ...
    void execute(std::vector<size_t> global_work, std::vector<size_t> local_work, 
		 cl_event* event = NULL) {
