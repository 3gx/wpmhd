...
//   NGBmax  = 16.1;
  ...
  const float3 box_rmax = (float3){2,  1, 1};
...
void system::set_problem(const bool init_data) {
  if (myid == 0) fprintf(stderr, "  *** advect_pulse problem *** \n");
  ...
  const float vel_x = 2.0;
  const float vel_y = 1.0;
  ...
  for (int i = 0; i < local_n; i++) {
    ...
    if (r < r0 &&  r > 0) {
      const float azx = amp * (x)/r;
      const float azy = amp * (y)/r;
      bx =  azy;
      by = -azx;
      bz = 0.0f;
      d  = 2.0f;
