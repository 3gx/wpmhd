...
struct ptcl_mhd {  // 10 64bit words, 8 mhd + 2 scalar 
  ...
  ptcl_mhd to_primitive(const float V) const {
    ptcl_mhd prim;
    const ptcl_mhd &cons = *this;
    prim.dens  = cons.mass/V;
    prim.vel.x = cons.mom.x/cons.mass;
    prim.vel.y = cons.mom.y/cons.mass;
    prim.vel.z = cons.mom.z/cons.mass;
    prim.B.x   = cons.wB.x/V;
    prim.B.y   = cons.wB.y/V;
    prim.B.z   = cons.wB.z/V;
  ...
  ptcl_mhd to_conservative(const float V) const  {
    ptcl_mhd cons;
    const ptcl_mhd &prim = *this;
    cons.mass  = prim.dens*V;
    cons.mom.x = prim.vel.x * cons.mass;
    cons.mom.y = prim.vel.y * cons.mass;
    cons.mom.z = prim.vel.z * cons.mass;
    cons.wB.x  = prim.B.x*V;
    cons.wB.y  = prim.B.y*V;
    cons.wB.z  = prim.B.z*V;

    cons.mscal  = prim.scal  * cons.mass;
    cons.mpsi  = prim.psi;
    cons.mpsi  = prim.psi * cons.mass;
#ifdef _CONSERVATIVE_
    cons.etot = prim.ethm*V
      + 0.5*(sqr(prim.vel.x) + sqr(prim.vel.y) + sqr(prim.vel.z))*cons.mass
      + 0.5*(sqr(prim.B.x  ) + sqr(prim.B.y  ) + sqr(prim.B.z  ))*V;
#elif defined _SEMICONSERVATIVE_
    cons.etot = prim.ethm *V
      + 0.5*(sqr(prim.B.x  ) + sqr(prim.B.y  ) + sqr(prim.B.z  ))*V;
#else
