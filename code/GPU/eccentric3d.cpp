...
//   const float z = pi.pos.z.getu() - gravity_pos.z; 
  ...
  mi.ethm = sqr(cs) * mi.dens;
    ...
    float cs1 = cs;
    cs1 = CS1;
    if (gamma_gas > 1.0f) mi.ethm = sqr(cs1) * mi.dens /(gamma_gas - 1.0)/gamma_gas;
  /*****************************/
  /*****************************/
  /*****************************/
...
//   do_kepler_drift = false;
  
  gravity_mass = Mbh;
  gravity_eps2 = 0*sqr(0.25f);
  gravity_rin  = Rin  * 0.1; //0.25f;
...
//   gravity_rin  = 0;   //Rin  * 0.25f;
  ...
  for (int i = 0; i < local_n; i++) {
    ...
    const float cosph = x/R;
    const float sinph = y/R;
    const float aaa   = R / (1-sqr(ecc))*(1 + ecc*cosph);
    aaa_max = std::max(aaa_max, aaa);
  }
  aaa_max *= 1.05f;
  ...
  for (int i = 0; i < local_n; i++) {
    ...
    const float sinph = y/R;
    const float aaa   = R / (1-sqr(ecc))*(1 + ecc*cosph);
    const float vR = ecc*sinph*sqrt(gravity_mass/r/(1 + ecc*cosph));
    const float vt =           sqrt(gravity_mass/r*(1 + ecc*cosph));
    const float vZ = 0.0;
#else
    const float vr = ecc*sinph*sqrt(gravity_mass/r/(1 + ecc*cosph));
    const float vt =           sqrt(gravity_mass/r*(1 + ecc*cosph));
    const float costh = z/r;
    const float sinth = r/R;
    const float cosph = x/R;
    const float sinph = y/R;

    /// **** 3D DISK NEEDED TO BE SET ... ***

    const float vZ = vr * costh;
    const float vR = vr * sinth;
#endif
    ...
    vx = vR*cosph - vt*sinph;
    vy = vR*sinph + vt*cosph;
    
    vz = vZ;
    
    const float vorb = sqrt(sqr(vR) + sqr(vt));
    const float dv = vorb * 1.0e-4;
    vx += drand48() * dv;
    vy += drand48() * dv;
...
//     bz = b0 * 2.0*(2.0*(n & 1) - 1);
    ...
    const float zzz = 1.0 - std::sqrt(aaa/aaa_max);
    if (zzz <= 0.0) 
      fprintf(stderr, "i= %d : %g %g  aaa= %g  zzz= %g\n",
	      i, x, y, aaa, zzz);
    assert(zzz > 0.0);
    assert(gamma_gas > 1.0f);
    const float n = 1.0f/(gamma_gas - 1.0f);
    float dmid = 1*std::pow(zzz, n);

//     dmid = dmid * std::pow(aaa/1.0, -1.0f);
    d = dmid * std::max(exp(-sqr(z)/2/sqr(H)), DENS_MIN);
    float cs1 = cs;
    cs1 = CS1;
    if (gamma_gas > 1.0f)  e = sqr(cs1) * d /(gamma_gas - 1.0)/gamma_gas;
    else                   e = sqr(cs1) * d;

    
//     d = std::pow(std::max(dmid - sqr(z/H)*(n+1), 1.0e-4f), n);
...
//   const float3 box_rmax = (float3){16.0f, 16.0f,  6.0f};
  ...
  if (myid == 0) {
	...
	while (flag) {
	  ...
	  const float vmin = sqrt(Mbh*(1-ecc)/(1+ecc)/aaa);
	  const float vr   = ecc*sin(phi) * sqrt(Mbh/r/(1+ecc*cos(phi)));
	  const float vt   =                sqrt(Mbh/r*(1+ecc*cos(phi)));
	  const float vorb = sqrt(sqr(vr) + sqr(vt));
	  	  
	  const float fv = drand48();
	  const float area_scale = r / (aaa*(1.0+ecc));
	  const float      scale = vmin/vorb * area_scale;

	  if (ecc == 0.0f) assert(scale == area_scale);
