...
ptcl_mhd system::solve_riemann(const float3 &Wij, const float3 &Bij, 
  ...
  const float  wij = sqrt(dwij.x*dwij.x + dwij.y*dwij.y + dwij.z*dwij.z);
  const float iwij = (wij > 0.0f) ? 1.0f/wij : 0.0f;
	    
  const float3 e = {dwij.x * iwij, dwij.y * iwij, dwij.z * iwij};
...
void  riemann_solver(const float w, const float b, const float v, const float gamma_gas,
  ...
  const REAL Bx = b;
  const REAL signBx = (std::abs(Bx) > 0.0) ? Bx/std::abs(Bx) : 0.0;
  ...
  const REAL cfl = sqrt((gpbl + sqrt( sqr(gpbl) - 4*gpl*sqr(Bx) ))/(2.0*dens_L));
  const REAL cfr = sqrt((gpbr + sqrt( sqr(gpbr) - 4*gpr*sqr(Bx) ))/(2.0*dens_R));
  const REAL cfmax = sqrt(1.0)*std::max(cfl,cfr);
  ...
  const REAL pT_L = pres_L + 0.5 * B2_L;
  const REAL pT_R = pres_R + 0.5 * B2_R;
  ...
  const REAL    S_L_s  = S_M - std::abs(Bx)/sqrt(dens_L_s);
  const REAL    S_R_s  = S_M + std::abs(Bx)/sqrt(dens_R_s);

  const REAL idsqrt  = 1.0/(sqrt(dens_L_s) + sqrt(dens_R_s));
